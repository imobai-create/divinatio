...
# Compila o contrato ANTES de subir a blockchain. Na primeira vez isso pode
# baixar o compilador e levar um minuto; fazer aqui (e não dentro do node)
# evita que a blockchain estoure o tempo de espera por estar compilando.
echo "🔨 Preparando o contrato (pode demorar na primeira vez)..."
if ! npx hardhat compile; then
  echo "❌ Falha ao compilar o contrato. Verifique sua conexão e tente de novo."
  exit 1
fi
...
# espera a blockchain responder (até 90s, com folga para máquinas mais lentas)
i=0
until curl -s -X POST -H 'Content-Type: application/json' \
  ...
  i=$((i + 1))
  if [ "$i" -ge 90 ]; then
    echo "❌ A blockchain local não respondeu. Últimas linhas do erro:"
    echo "─────────────────────────────────────────"
    tail -20 /tmp/divinatio-node.log
    echo "─────────────────────────────────────────"
    exit 1
  fi
  sleep 1
